...
def _error_report(stats: Dict[str, Any]) -> str:
    ...
    lines.append("\u2500" * 48)
    lines.append(f"  Total requests:  {_fmt_num(stats['total_requests'])}")
    lines.append(f"  Error count:     {_fmt_num(stats.get('error_count', 0))}")
    lines.append(f"  Error rate:      {stats.get('error_rate', 0):.1f}%")
    lines.append("")
